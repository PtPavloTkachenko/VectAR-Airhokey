...
func SSHSetup(w http.ResponseWriter, r *http.Request) {
	...
	case r.URL.Path == "/api-ssh/setup":
		...
		if keyPath != "" {
			...
			if err != nil {
				fmt.Fprint(w, "error: cannot read ssh key at "+keyPath+" ("+err.Error()+")")
				return
			}
		} else {
			key, _, err := r.FormFile("key")
			if err != nil {
				fmt.Fprint(w, "error: must provide ssh key or key_path ("+err.Error()+")")
				return
			}
			keyBytes, _ = io.ReadAll(key)
		}
		if len(keyBytes) < 5 {
			fmt.Fprint(w, "error: ssh key is empty or too short")
			return
		}
