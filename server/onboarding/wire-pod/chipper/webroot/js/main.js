...
function checkKG() {
  ...
  if (provider) {
    if (provider === "houndify") {
      ...
      getE("intentGraphInput").style.display = "block";
      getE("googleInput").style.display = "block";
      getE("saveChatInput").style.display = "block";
      getE("llmCommandInput").style.display = "block";
      ...
      getE("intentGraphInput").style.display = "block";
      getE("claudeInput").style.display = "block";
      getE("saveChatInput").style.display = "block";
      getE("llmCommandInput").style.display = "block";
    }
  }
...
function sendKGAPIKey() {
  ...
  if (provider === "openai") {
    ...
    data.key = getE("googleKey").value;
    data.model = getE("googleModel").value;
    data.openai_prompt = getE("googleAIPrompt").value;
    data.endpoint = "https://generativelanguage.googleapis.com/v1beta";
    data.intentgraph = getE("intentyes").checked;
    data.save_chat = getE("saveChatYes").checked;
    data.commands_enable = getE("commandYes").checked;
  } else if (provider === "custom") {
    ...
    data.model = getE("claudeModel").value;
    data.openai_prompt = getE("claudeAIPrompt").value;
    data.intentgraph = getE("intentyes").checked;
    data.save_chat = getE("saveChatYes").checked;
    data.commands_enable = getE("commandYes").checked;
  } else {
  ...
  fetch("/api/get_kg_api")
    ...
    .then((data) => {
      ...
      if (data.provider === "openai") {
        ...
        getE("googleKey").value = data.key;
        getE("googleModel").value = data.model;
        getE("googleAIPrompt").value = data.openai_prompt;
        getE("commandYes").checked = data.commands_enable;
        getE("intentyes").checked = data.intentgraph;
        getE("saveChatYes").checked = data.save_chat;
        ...
        getE("claudeModel").value = data.model;
        getE("claudeAIPrompt").value = data.openai_prompt;
        getE("commandYes").checked = data.commands_enable;
        getE("intentyes").checked = data.intentgraph;
        getE("saveChatYes").checked = data.save_chat;
      }
...
function setSTTProvider() {
  const provider = getE("sttProviderSelect").value;
  displayMessage("sttProviderStatus", "Setting...");
  fetch("/api/set_stt_provider", {
    method: "POST",
    ...
    .then((response) => response.text())
    .then((response) => {
      displayMessage("sttProviderStatus", response);
    });
}

function loadSTTProvider() {
  fetch("/api/get_stt_info")
    .then((response) => response.json())
    .then((parsed) => {
      if (parsed.transcribe_with) {
        getE("sttProviderSelect").value = parsed.transcribe_with;
      } else {
        getE("sttProviderSelect").value = "vosk";
      }
    });
}

function loadVoskModels() {
  fetch("/api/get_vosk_models")
    .then((response) => response.json())
    .then((models) => {
      const select = getE("voskModelSelect");
      select.innerHTML = "";
      ...
      models.forEach((m) => {
        const opt = document.createElement("option");
        opt.value = m.name;
        opt.text = m.name + (m.current ? " (active)" : "");
...
function setVoskModel() {
  const model = getE("voskModelSelect").value;
  if (!model) return;
  displayMessage("voskModelStatus", "Switching model and reloading VOSK...");
  fetch("/api/set_vosk_model", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    ...
    .then((response) => response.text())
    .then((response) => {
      displayMessage("voskModelStatus", response);
      loadVoskModels();
    });
}
...
function showLanguage() {
  toggleVisibility(["section-weather", "section-restart", "section-kg", "section-language", "section-preferences"], "section-language", "icon-Language");
  loadSTTProvider();
  fetch("/api/get_stt_info")
    .then((response) => response.json())
    .then((parsed) => {
      if (parsed.provider !== "vosk" && parsed.provider !== "whisper.cpp") {
        ...
        getE("voskModelDiv").style.display = "none";
      } else {
        getE("languageSelectionDiv").style.display = "block";
        getE("languageSelection").value = parsed.language;
        if (parsed.provider === "vosk") {
          getE("voskModelDiv").style.display = "block";
          loadVoskModels();
        } else {
          getE("voskModelDiv").style.display = "none";
        }
...
function showWeather() {
  toggleVisibility(["section-weather", "section-restart", "section-language", "section-kg", "section-preferences"], "section-weather", "icon-Weather");
}

function showKG() {
  toggleVisibility(["section-weather", "section-restart", "section-language", "section-kg", "section-preferences"], "section-kg", "icon-KG");
}

function showPreferences() {
  toggleVisibility(["section-weather", "section-restart", "section-language", "section-kg", "section-preferences"], "section-preferences", "icon-Preferences");
  loadPreferences();
}

function loadPreferences() {
  fetch("/api/get_preferences")
    .then(r => r.json())
    .then(data => {
      document.getElementById("prefLocation").value = data.location || "";
      document.getElementById("prefTempUnit").value = data.temperature_unit || "C";
      document.getElementById("prefTimezone").value = data.timezone || "";
    })
...
function savePreferences() {
  var body = {
    ...
    temperature_unit: document.getElementById("prefTempUnit").value,
  ...
  fetch("/api/set_preferences", {
    ...
    .then(r => r.text())
    .then(text => {
      document.getElementById("preferencesStatus").innerHTML = "<p>" + text + "</p>";
    })
    .catch(err => {
      document.getElementById("preferencesStatus").innerHTML = "<p>Error: " + err + "</p>";
    });
}
