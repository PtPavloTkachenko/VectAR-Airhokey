...
// ChatStream is a common interface for OpenAI and Gemini streaming
...
func isGeminiEndpoint(endpoint string) bool {
	return strings.Contains(endpoint, "generativelanguage.googleapis.com")
}

// GeminiSearchStream implements ChatStream using Gemini native API with Google Search grounding
type GeminiSearchStream struct {
	reader *bufio.Reader
	resp   *http.Response
	done   bool
}

func NewGeminiSearchStream(ctx context.Context, messages []openai.ChatCompletionMessage, model, apiKey, endpoint string) (*GeminiSearchStream, error) {
	// Build Gemini native streaming URL
	streamURL := fmt.Sprintf("%s/models/%s:streamGenerateContent?key=%s&alt=sse",
	...
	logger.Println("Gemini Search request to: " + streamURL)

	req, err := http.NewRequestWithContext(ctx, "POST", streamURL, bytes.NewReader(jsonBody))
	if err != nil {
	...
	return &GeminiSearchStream{
		reader: bufio.NewReader(resp.Body),
		resp:   resp,
	}, nil
}

func (s *GeminiSearchStream) Recv() (openai.ChatCompletionStreamResponse, error) {
	if s.done {
...
func (s *GeminiSearchStream) Close() error {
	if s.resp != nil {
...
func StreamingKGSim(req interface{}, esn string, transcribedText string, isKG bool) (string, error) {
	...
	geminiEndpoint := vars.APIConfig.Knowledge.Endpoint
	if vars.APIConfig.Knowledge.Provider == "google" && geminiEndpoint == "" {
		geminiEndpoint = "https://generativelanguage.googleapis.com/v1beta"
	}
	if vars.APIConfig.Knowledge.Provider == "google" || (vars.APIConfig.Knowledge.Provider == "custom" && isGeminiEndpoint(vars.APIConfig.Knowledge.Endpoint)) {
		// Use Gemini native API with Google Search grounding
		logger.Println("Using Gemini with Google Search grounding")
		logger.LogUI("Using Gemini + Google Search for " + esn)
		gemStream, gerr := NewGeminiSearchStream(ctx, aireq.Messages,
			vars.APIConfig.Knowledge.Model,
			vars.APIConfig.Knowledge.Key,
			geminiEndpoint,
		)
		if gerr != nil {
			log.Printf("Gemini Search stream error: %v", gerr)
			if isKG {
				kgStopLooping = true
		...
		chatStream = gemStream
	} else if vars.APIConfig.Knowledge.Provider == "claude" {
		// Use Claude CLI (no API key needed, authenticated via `claude auth`)
		model := vars.APIConfig.Knowledge.Model
		if model == "" {
			model = "haiku"
		}
		logger.Println("Using Claude CLI for " + esn)
		logger.LogUI("Using Claude CLI (model: " + model + ") for " + esn)
		claudeStream, cerr := NewClaudeCLIStream(ctx, aireq.Messages, model)
		if cerr != nil {
			log.Printf("Claude CLI stream error: %v", cerr)
			if isKG {
				kgStopLooping = true
				for range kgReadyToAnswer {
					break
		...
		stream, serr := c.CreateChatCompletionStream(ctx, aireq)
		if serr != nil {
			log.Printf("Error creating chat completion stream: %v", serr)
			if strings.Contains(serr.Error(), "does not exist") && vars.APIConfig.Knowledge.Provider == "openai" {
				...
				aireq2 := CreateAIReq(transcribedText, esn, true, isKG)
				logger.Println("Falling back to " + aireq2.Model)
				logger.LogUI("Falling back to " + aireq2.Model)
				stream, serr = c.CreateChatCompletionStream(ctx, aireq2)
				if serr != nil {
