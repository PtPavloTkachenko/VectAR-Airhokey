...
const (
	...
	ActionPlaySound = 5
	// movement commands
	ActionDriveForward  = 6
	ActionDriveBackward = 7
	ActionTurnLeft      = 8
	ActionTurnRight     = 9
	ActionLookUp        = 10
	ActionLookDown      = 11
	ActionDockCharger   = 12
)
...
var ValidLLMCommands []LLMCommand = []LLMCommand{
	...
	// },
	{
		...
		Description:     "Drives the robot forward. Use 'short' for ~10cm, 'medium' for ~25cm, 'long' for ~50cm.",
		ParamChoices:    "short, medium, long",
		Action:          ActionDriveForward,
		SupportedModels: []string{"all"},
	},
	{
		Command:         "driveBackward",
		Description:     "Drives the robot backward. Use 'short' for ~10cm, 'medium' for ~25cm, 'long' for ~50cm.",
		ParamChoices:    "short, medium, long",
		Action:          ActionDriveBackward,
		SupportedModels: []string{"all"},
	},
	{
		Command:         "turnLeft",
		Description:     "Turns the robot left. Use 'slight' for ~45 degrees, 'normal' for ~90 degrees, 'around' for ~180 degrees.",
		ParamChoices:    "slight, normal, around",
		Action:          ActionTurnLeft,
		SupportedModels: []string{"all"},
	},
	{
		Command:         "turnRight",
		Description:     "Turns the robot right. Use 'slight' for ~45 degrees, 'normal' for ~90 degrees, 'around' for ~180 degrees.",
		ParamChoices:    "slight, normal, around",
	...
	{
		Command:         "lookUp",
		Description:     "Tilts the robot's head up to look upward.",
		ParamChoices:    "now",
		Action:          ActionLookUp,
		SupportedModels: []string{"all"},
	},
	{
		Command:         "lookDown",
		Description:     "Tilts the robot's head down to look at the ground or nearby objects.",
		ParamChoices:    "now",
	...
	{
		Command:         "goToCharger",
		Description:     "Makes the robot drive back to its charger. Use when asked to go home, rest, or charge.",
		ParamChoices:    "now",
		Action:          ActionDockCharger,
		SupportedModels: []string{"all"},
	},
}
...
func CreatePrompt(origPrompt string, model string, isKG bool) string {
	...
	var prefs []string
	if vars.APIConfig.Preferences.Location != "" {
		prefs = append(prefs, "User location: "+vars.APIConfig.Preferences.Location)
	}
	if vars.APIConfig.Preferences.TemperatureUnit == "F" {
		prefs = append(prefs, "Use Fahrenheit for temperature.")
	} else {
		prefs = append(prefs, "Use Celsius for temperature.")
	}
	if vars.APIConfig.Preferences.TimeZone != "" {
		prefs = append(prefs, "User timezone: "+vars.APIConfig.Preferences.TimeZone)
	}
	if len(prefs) > 0 {
		prompt = prompt + "\n\n" + strings.Join(prefs, " ")
	}
...
func DoSayText(input string, robot *vector.Vector) error {
	...
	logger.Println("TTS speaking: " + input)
	...
	if (vars.APIConfig.STT.Language != "en-US" && vars.APIConfig.Knowledge.Provider == "openai") || vars.APIConfig.Knowledge.OpenAIVoiceWithEnglish {
		...
		if err != nil {
			logger.Println("TTS OpenAI error: " + err.Error())
		}
		return err
	}
	...
	if err != nil {
		logger.Println("TTS SayText error: " + err.Error())
	}
	return err
}
...
var driveDistances = map[string]float32{
	"short":  100,
	"medium": 250,
	"long":   500,
}

var turnAngles = map[string]float32{
	"slight": 45,
	"normal": 90,
	"around": 180,
}

func ensureOffCharger(robot *vector.Vector) {
	resp, err := robot.Conn.BatteryState(context.Background(), &vectorpb.BatteryStateRequest{})
	if err != nil {
		return
	}
	if resp.IsOnChargerPlatform {
		logger.Println("Robot is on charger — driving off first")
		robot.Conn.DriveOffCharger(context.Background(), &vectorpb.DriveOffChargerRequest{})
		time.Sleep(time.Second * 2)
		// Drive forward a bit to clear the dock
		robot.Conn.DriveStraight(context.Background(), &vectorpb.DriveStraightRequest{
			SpeedMmps: 100,
			DistMm:    120,
		})
		time.Sleep(time.Second * 2)
...
func DoDrive(param string, forward bool, robot *vector.Vector) error {
	ensureOffCharger(robot)
	dist, ok := driveDistances[param]
	if !ok {
		dist = 250
	}
	if !forward {
	...
	logger.Println(fmt.Sprintf("Drive: dist=%.0fmm speed=%.0fmm/s", dist, speed))
	_, err := robot.Conn.DriveStraight(context.Background(), &vectorpb.DriveStraightRequest{
		SpeedMmps: speed,
		DistMm:    dist,
	})
	return err
...
func DoTurn(param string, left bool, robot *vector.Vector) error {
	ensureOffCharger(robot)
	angle, ok := turnAngles[param]
	if !ok {
		angle = 90
	}
	if left {
		angle = -angle // negative = left in Vector SDK
	}
	logger.Println(fmt.Sprintf("Turn: angle=%.0f degrees", angle))
	_, err := robot.Conn.TurnInPlace(context.Background(), &vectorpb.TurnInPlaceRequest{
...
func DoLookUpDown(up bool, robot *vector.Vector) error {
	angle := float32(20) // degrees up
	if !up {
		angle = -10 // degrees down
	}
	logger.Println(fmt.Sprintf("Head angle: %.0f degrees", angle))
	_, err := robot.Conn.SetHeadAngle(context.Background(), &vectorpb.SetHeadAngleRequest{
		AngleRad: angle * 3.14159 / 180,
	})
	return err
}

func DoDockCharger(robot *vector.Vector) error {
	logger.Println("Driving to charger...")
...
func DoGetImage(msgs []openai.ChatCompletionMessage, param string, robot *vector.Vector, stopStop chan bool) {
	...
	// For Claude CLI: save image to temp file and use CLI with Read tool
	if vars.APIConfig.Knowledge.Provider == "claude" {
		imgPath := "/tmp/vector_camera.jpg"
		os.WriteFile(imgPath, resp.Data, 0644)
		logger.Println("Saved camera image to " + imgPath)
		...
		visionPrompt := "Look at the image at " + imgPath + " and describe what you see. Be concise (2-3 sentences max). No special characters."
		cmd := exec.CommandContext(context.Background(), claudeBin,
			"-p", "--model", model,
			"--allowedTools", "Read",
		)
		cmd.Stdin = strings.NewReader(visionPrompt)
		cmd.Env = filterEnv(os.Environ(), "CLAUDECODE")
		out, err := cmd.Output()
		if err != nil {
			logger.Println("Claude vision error: " + err.Error())
			DoSayText("I could not analyze the image.", robot)
			return
		}
		response := strings.TrimSpace(string(out))
		logger.Println("Claude vision response: " + response)
		DoSayText(response, robot)
		return
	}
...
func PerformActions(msgs []openai.ChatCompletionMessage, actions []RobotAction, robot *vector.Vector, stopStop chan bool) bool {
	...
	for _, action := range actions {
		...
		case action.Action == ActionDriveForward:
			DoDrive(action.Parameter, true, robot)
		case action.Action == ActionDriveBackward:
			DoDrive(action.Parameter, false, robot)
		case action.Action == ActionTurnLeft:
			DoTurn(action.Parameter, true, robot)
		case action.Action == ActionTurnRight:
			DoTurn(action.Parameter, false, robot)
		case action.Action == ActionLookUp:
			DoLookUpDown(true, robot)
		case action.Action == ActionLookDown:
			DoLookUpDown(false, robot)
		case action.Action == ActionDockCharger:
			DoDockCharger(robot)
		}
	}
