...
func ProcessTextAll(req interface{}, voiceText string, intents []vars.JsonIntent, isOpus bool) bool {
	...
	if !customIntentMatched && !pluginMatched {
		...
		// Skip weather intents if Weather API not configured — let LLM handle it
		skipWeather := (!vars.APIConfig.Weather.Enable || vars.APIConfig.Weather.Key == "") && vars.APIConfig.Knowledge.Enable
		// Look for a perfect match first
		for _, b := range intents {
			if skipWeather && strings.Contains(b.Name, "intent_weather") {
				continue
			}
